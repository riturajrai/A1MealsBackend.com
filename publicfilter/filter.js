    ...
    if (withCount === "true") {
      const withCounts = await Promise.all(
        categories.map(async (cat) => {
          const mealCount = await Meal.countDocuments({category: cat._id,status: "active",isAvailable: true, isDeleted: false});
...
router.get("/categories/slug/:slug", async (req, res) => {
  try {
    const category = await Category.findOne({ slug: req.params.slug }).select("-__v").lean();
    if (!category) {
      return res.status(404).json({ success: false, message: "Category not found" });
    }
    res.json({success: true, category });
...
router.get("/food-types", async (req, res) => {
  try {
    const foodTypes = await FoodType.find().select("-__v -createdBy")
      .sort({ name: 1 }).lean();
...
router.get("/food-types/:id", async (req, res) => {
  try {
    const foodType = await FoodType.findById(req.params.id).select("-__v -createdBy").lean();
    if (!foodType) {
...
router.get("/tags", async (req, res) => {
  try {
    ...
    const query = search?.trim() ? { name: { $regex: search.trim(), $options: "i" } }: {};
    const tags = await Tags.find(query).select("-__v -createdBy").sort({ name: 1 }).lean();
...
router.get("/tags/:id", async (req, res) => {
  try {
    const tag = await Tags.findById(req.params.id).select("-__v -createdBy").lean();
    if (!tag) {
...
router.get("/filters", async (req, res) => {
  try {
    ...
    res.json({success: true,categories,foodTypes,tags});
  } catch (err) {
